...
function plantillaCorreoRecuperacion(nombreCompleto, urlRestablecer) {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px; background-color: #f7f9fc;">
        ...
        <div style="background-color: #ffffff; border-radius: 16px; padding: 32px 24px; box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
            <p style="color: #333333; font-size: 15px;">Hola, <strong>${nombreCompleto}</strong>:</p>
            <p style="color: #333333; font-size: 15px; line-height: 1.5;">
                Recibimos una solicitud para restablecer la contraseña de tu cuenta en Volare Hub.
                Si fuiste tú, haz clic en el siguiente botón para continuar:
            </p>
            <div style="text-align: center; margin: 32px 0;">
                <a href="${urlRestablecer}" style="background-color: #43B02A; color: #ffffff; text-decoration: none; font-weight: bold; padding: 12px 28px; border-radius: 8px; display: inline-block;">
...
async function enviarCorreoRecuperacion(usuario, token) {
    ...
    await resend.emails.send({
        from: 'Volare Hub <noreply@urbvolare.com>',
        to: usuario.email,
        subject: 'Restablece tu contraseña - Volare Hub',
